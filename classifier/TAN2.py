...
class TanBayes2:
    ...
    def probability_precal(self):
        self.priorAbove50K = len(self.attrAbove50KDict) / (self.totalAbove50K+self.totalUnder50K)
        self.priorUnder50K = len(self.attrUnder50KDict) / (self.totalAbove50K+self.totalUnder50K)
    ...
    def continus_continus_info(self, i, j, is_above):
        attrDict, totalNum = self.dictNtotal(is_above)
        varX, varY, eX, eY, p = self.Cov_roll(i, j, attrDict, totalNum)
        return -0.5*np.log(1-p*p)

    # 辅助函数： 计算协方差率
    def Cov_roll(self, i, j, attrDict, totalNum):
        ...
        Cov_xy /= totalNum
        return varX, varY, eX, eY, Cov_xy / np.sqrt(varX * varY)
        ...
        for i in range(0, const.IF_OVER_50K):
            ...
            if i == const.AGE or i == const.HOURS_PER_WEEK:             # 连续型 服从高斯分布
                ...
                if parent == -1:                                        # 无父类
                    ...
                    varX, varY, eX, eY, p = self.Cov_roll(i, parent, attrDict, totalNum)
                    tmp = 1-p*p
                ...
                if parent == -1:                                        # 无父类
                    ...
                    for d in attrDict:
                        if float(d[i]) == -1:
                            con0 += float(data[const.FNLWGT])
                    if float(data[i]) == -1:
                        pro = con0 / totalNum
                        rate *= pro
                    else:
                        pro = (1-con0)/totalNum * self.Gauss_pro(float(data[i]), self.continus[i]['aver' + str(is_above)],
                                        self.continus[i]['var' + str(is_above)])
                        rate *= pro
                elif not is_continuous(parent):                         # 父类 离散型
                    con0, list = 0, []
                    ...
                    if con0+total == 0:
                        ...
                        if float(data[i]) == -1:
                            ...
                            pro = (1-con0)/(con0+total) * self.Gauss_pro(float(data[i]), self.continus[i]['aver' + str(is_above)],
                                            self.continus[i]['var' + str(is_above)])
                            rate *= pro
