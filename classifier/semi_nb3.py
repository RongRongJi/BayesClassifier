        ...
        if self.totalUnder50K == 0:
            ...
            for i in range(0,const.IF_OVER_50K):
                ...
                if i == const.AGE or i == const.HOURS_PER_WEEK:
                    ...
                    average = average / (self.totalUnder50K + 1)
                    variance = 0
                    for key, value in self.attrUnder50KDict[i].items():
                    ...
                    if '-1' not in self.attrUnder50KDict[i]:
                        construct_zero = 1 / (self.totalUnder50K + 1)
                    else:
                        construct_zero = self.attrUnder50KDict[i]['-1'] / (self.totalUnder50K + 1)
                    # 判断是否是结构0
                    if data[i] == '-1':
            ...
            for i in range(0, const.IF_OVER_50K):
                ...
                if i == const.AGE or i == const.HOURS_PER_WEEK:
                    ...
                    for key, value in self.attrAbove50KDict[i].items():
                        average = average + float(key) * value
                    average = average / (self.totalAbove50K + 1)
                    variance = 0
                    for key, value in self.attrAbove50KDict[i].items():
                        variance = variance + value * np.power(float(key) - average, 2)
                    ...
                    if '-1' not in self.attrAbove50KDict[i]:
                        construct_zero = 1 / (self.totalAbove50K + 1)
                    else:
                        construct_zero = self.attrAbove50KDict[i]['-1'] / (self.totalAbove50K + 1)
                    # 判断是否是结构0
                    if data[i] == '-1':
